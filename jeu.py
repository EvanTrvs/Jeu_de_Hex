...
class HEX:
    ...
    def ecran_accueil(self, *event): #Focntion créatrice de l'écran d'accueil
        ...
        play.config(command = self.ecran_parametres) #Configuration du bouton de lancement de jeu
    ...
    def button_config(self, save1, save2, save3, restart, tomenu): #Fonction de configuration des sauvegardes
        ...
        save1 [0].config (save1 [1].entryconfig ("Enregistrer sur A", command = lambda: [enregistrement (self.plat, self.ordre, self.players, self.bot, self.actual_time(), 1),self.ecran_sauvegarde(save1, save2, save3)])) #Configuration du bouton d'enregistrement A avec le plateau, l'ordre, les joueurs ou bot, le temps mais également l'actualisation du chargement 
        save2 [0].config (save2 [1].entryconfig ("Enregistrer sur B", command = lambda: [enregistrement (self.plat, self.ordre, self.players, self.bot, self.actual_time(), 2),self.ecran_sauvegarde(save1, save2, save3)])) #Configuration du bouton d'enregistrement B avec le plateau, l'ordre, les joueurs ou bot, le temps mais également l'actualisation du chargement
        save3 [0].config (save3 [1].entryconfig ("Enregistrer sur C", command = lambda: [enregistrement (self.plat, self.ordre, self.players, self.bot, self.actual_time(), 3),self.ecran_sauvegarde(save1, save2, save3)])) #Configuration du bouton d'enregistrement C avec le plateau, l'ordre, les joueurs ou bot, le temps mais également l'actualisation du chargement
